...
for await (const line of readLines(Deno.stdin)) {
  for (let j = 0; j < line.length; j += 1) {
    switch (line[j]) {
      ...
      case "C":
      case "D":
        initialMap.set([j, i].join(","), line[j]);
        break;
      default:
    }
  }
...
const canEntry = function (map, pod, x, y) {
  ...
  if (y === 2) return map.get(key(x, y + 1)) === pod;
  return true;
};
...
const successors = function (state) {
  ...
  for (const [pkey, pod] of map.entries()) {
    ...
    for (const [m, , c] of moves(map, p)) {
      ss.push({ map: m, cost: cost + c, h: h1(m) });
    }
  }
  return ss;
};
...
const isGoal = function (state) {
  const { map } = state;
  return map.get("3,2") === "A" && map.get("3,3") === "A" &&
    map.get("5,2") === "B" && map.get("5,3") === "B" &&
    map.get("7,2") === "C" && map.get("7,3") === "C" &&
    map.get("9,2") === "D" && map.get("9,3") === "D";
};
...
const findBest = function (state) {
  ...
  while (ss.length) {
    ...
    successors(current).forEach((s) => ss.push(s));
    visited.add(sc);
  }
};
