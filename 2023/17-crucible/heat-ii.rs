...
fn is_goal(trip: &Trip) -> bool {
    ...
    trip.position == (width - 1, height - 1) && trip.min_straight == 0
}
...
fn successors(trip: &Trip, heatmap: &HeatMap) -> Vec<Trip> {
    ...
    [(1, 0), (0, -1_i32), (0, 1), (-1_i32, 0)].iter()
        .filter_map(|(dx, dy)| {
            ...
            let (width, height) = trip.size;
            let mut min_straight = trip.min_straight;
            let mut max_straight = trip.max_straight;

            if nx >= width || ny >= height { return None };

            if trip.direction == (-dx, -dy) { return None };
            if trip.direction == (*dx, *dy) {
                if min_straight > 0 { min_straight -= 1 };
                max_straight -= 1;
                if max_straight == 0 { return None };
            } else {
                if min_straight > 0 { return None };
                min_straight = 3;
                max_straight = 10;
            }
            ...
            t.position = (nx, ny);
            t.direction = (*dx, *dy);
            t.min_straight = min_straight;
            t.max_straight = max_straight;
